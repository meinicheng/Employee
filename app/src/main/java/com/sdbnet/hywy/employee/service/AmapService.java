...
public class AmapService extends Service {
    ...
    private static final String TAG = "AmapService";
    ...
    private void openScreenLockLocatioin() {
        if (mScreenLocak == null) {
            mScreenLocak = ScreenLockLocation.getInstance().init(
                    AmapService.this, new BroadcastReceiver() {
                        @Override
...
//            }
    ...
    private Runnable mAmapRunnable=new Runnable() {
        @Override
        public void run() {
    ...
    private void requestAmapLocation() {
        ...
        if (UtilsAndroid.Set.checkNetState(this)) {
            ...
            if (mAmapLocType.equals(LocationManagerProxy.GPS_PROVIDER)) {
                mHandler.postDelayed(mAmapRunnable, Constants.Value.DELAY_TIME_MIDDLE);
            }
    ...
    private BroadcastReceiver mGPSLocationReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(GPSLOCATION_BROADCAST_ACTION)) {
                ...
                if (location == null) {
                    if (UtilsAndroid.Set
                            .checkNetState(AmapService.this)) {
                        requestAmapLocation(LocationProviderProxy.AMapNetwork);
                    } else {
                        callBackFail();
                    }
...
//        isStartWeatherThread = false;
    ...
    private MyAmpListener mAMapLocationListener = new MyAmpListener() {
        ...
        public void onLocationChanged(AMapLocation amapLocation) {
            mHandler.removeCallbacks(mAmapRunnable);
...
//        stopGpsService();
    ...
    private boolean checkErrorCode(JSONObject response) {
        ...
        try {
            ...
            e.printStackTrace();
            ErrLogUtils.uploadErrLog(AmapService.this, (e));
            return false;
        }
        return false;
    }
    ...
    private class MyGeoListener implements OnGetGeoCoderResultListener {
        ...
        public void onGetReverseGeoCodeResult(ReverseGeoCodeResult reverseResult) {
            ...
            ToastUtil.show(AmapService.this,
            ...
            if (TextUtils.isEmpty(address)) {
                ...
                callBack(Constants.Location.ERROR_CODE_REVERSE_GEO);
                String msg = String.format(AmapService.this.getString(R.string.gps_location_empty_msg),
                        reverseResult.getLocation().latitude + "",
                        reverseResult.getLocation().longitude + "");

                ErrLogUtils.uploadErrLog(AmapService.this, msg);
...
//        0：离线定位请求成功 1:service没有启动 2：无监听函数 6：两次请求时间太短
    ...
    private void dealBDLocation(BDLocation location) {
        if (TextUtils.isEmpty(location.getAddrStr())) {
            ...
            // 上传定位
            upLocationData(location.getLongitude(), location.getLatitude(), address);
        ...
        public void onRegeocodeSearched(RegeocodeResult result, int rCode) {
            ...
            if (rCode == 0) {
                if (result != null
                    ...
                    ToastUtil.show(AmapService.this, "无地址");
                }
            } else {
                ToastUtil.show(AmapService.this, "无效" + rCode);
            }
        }
    }
